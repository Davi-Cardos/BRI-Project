...
def calcular_precision_recall(resultados, documentos_relevantes, k_values=[2, 4, 6, 8, 10]):
    ...
    for fonte in resultados[:10]:
        print(f" - Título: {fonte['title']}")
        print(f"   Caminho: {fonte['path']}")
        print(f"   Similaridade: {fonte['score']:.2f}")
...
def detectar_plagio_avaliacao(diretorio_suspeito, index_path="indexdir"):
    ...
    with index.searcher() as searcher:
        for doc in searcher.all_stored_fields():
            documentos_relevantes.append(doc["path"])
    ...
    for arquivo in arquivos_suspeitos:
        with open(arquivo, "r", encoding="utf-8") as file:
            ...
            if resultados:
                ...
                melhores_resultados.append(resultados[0])
...
if __name__ == "__main__":
    diretorio_suspeito = "dataset/external-detection-corpus/suspicious-document"
    index_dir = "indexdir"
